...
def score_box_loss(target_scores, target_deltas, pred_logits, pred_deltas):
    ...
    # pred_scores: [B, k]
    # pred_deltas: [B, k, 4]
    score_loss = sigmoid_focal_loss(pred_logits, target_scores, reduction='sum')
    pos_mask = target_scores > 0.5

    target_box_deltas_pos = target_deltas[pos_mask].reshape(-1, 2, 2)
    pred_box_deltas_pos = pred_deltas[pos_mask].reshape(-1, 2, 2)
    box_loss = euclidean_loss(pred_box_deltas_pos, target_box_deltas_pos, 'sum')
